...
function filter_create_deck(name,params) {
    ...
    for (var i=0;i<WL.length();i++){
        //If conditions
		if (WORDLISTMGR.active().current().Chapter == selections.chapter){
			console.log(WORDLISTMGR.active().current());
			word_box_html=wordbox(WORDLISTMGR.active().current());
	//        html2add=word2html(WORDLISTMGR.active().current())
			$('#FilterOutput').append(word_box_html)
		}
        next()
    }
}
